...
//처리는 성능을 많이 요구합니다
//또한 메모리를 정리하는 GC(가비지 컬렉선) 유발하기 쉽습니다.
...
public class PlatformSpawner : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int i = 0; i<count; i++)
        {
            ...
            //Quaternion.identity : 회전값 즉 오일러각의 (0,0,0)회전에 대응 = Quaternion.Euler(new Vector3(0,0,0));
    ...
    void Update()
    {
        ...
        {
            //기록된 마지막 배치 시점을 현재 시점으로 갱싱
            lastSpawnTime = Time.time;
            //다음 배치까지의 시간 간격을 timeBetSpawnMin, timeGetSpawnMax 사이에서 랜덤 설정
            timeBetSpawn = Random.Range(timeBetSpawnMin, timeBetSpawnMax);
            //배치할 위치의 높이를 yMin과 yMax 사이에서 랜덤 설정
            float yPos = Random.Range(yMin, yMax);
            //사용할 현재 순번의 발판 게임 오브젝트를 비활성화하고 바로 즉시 다시 활성화
            //왜냐하면 Platform 의 OnEnabel() 때문에
